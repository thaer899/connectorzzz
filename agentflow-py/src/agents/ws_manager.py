...
class WebSocketManagerAgent(GroupChatManager):
    def __init__(self, groupchat: GroupChat, name, llm_config, max_consecutive_auto_reply, send_queue,  receive_queue, **kwargs):
        super().__init__(groupchat, name, max_consecutive_auto_reply, **kwargs)
    ...
    def run_chat(
        ...
        #         if agent != sender:
        #             self.send(message, agent, request_reply=False, silent=True)
        #             logging.info(f"Message: {message} sent to agent: {agent.name}")

...
class ConversableAgent(Agent):
    ...
    def _append_oai_message(self, message: Union[Dict, str], role, conversation_id: Agent) -> bool:
        ...
        oai_message = {k: message[k] for k in (
            "content", "function_call", "name", "context") if k in message}
        if "content" not in oai_message:
            if "function_call" in oai_message:
                # if only function_call is provided, content will be set to None.
                oai_message["content"] = None
            else:
                return False

        oai_message["role"] = "function" if message.get(
            "role") == "function" else role
        if "function_call" in oai_message:
    ...
    def send(
        ...
        broadcast_message(message_details)
    ...
    async def a_send(
        ...
        # Broadcast the message
        await broadcast_message(message_details)
    ...
    def _print_received_message(self, message: Union[Dict, str], sender: Agent):
        ...
        print(colored(sender.name, "yellow"),
              "(to", f"{self.name}):\n", flush=True)
    ...
    def receive(
        ...
        reply = self.generate_reply(
            messages=self.chat_messages[sender], sender=sender)

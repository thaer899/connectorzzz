...
WORKING_DIR = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), "extensions")
UNKNOWN = "unknown"
TIMEOUT_MSG = "Timeout"
DEFAULT_TIMEOUT = 600
...
def run_code_in_process(queue, cmd, work_dir):
...
def execute_code(
    ...
    if not use_docker or in_docker_container:
        ...
        if WIN32:
            ...
            queue = Queue()
            process = Process(target=run_code_in_process,
                              args=(queue, cmd, work_dir))
            process.start()
        ...
        if result.returncode:
            ...
            if original_filename is None:
                ...
                abs_path = str(pathlib.Path(
                    work_dir).absolute()) + PATH_SEPARATOR
...
def eval_function_completions(
    ...
    if assertions is None:
        ...
        for i in range(n):
            ...
            success = execute_code(code, timeout=timeout,
                                   use_docker=use_docker)[0] == 0
    ...
    if n > 1 or test is None:
        for i in range(n):
            ...
            code = (
                f"{response}\n{assertions}" if response.startswith(
                    "def") else f"{definition}{response}\n{assertions}"
            )
            succeed_assertions = execute_code(
                code, timeout=timeout, use_docker=use_docker)[0] == 0
    ...
    success = execute_code(code_test, timeout=timeout,
                           use_docker=use_docker)[0] == 0
    return {
        "index_selected": i,
        "succeed_assertions": succeed_assertions,
...
_IMPLEMENT_CONFIGS = [
    {"model": FAST_MODEL, "prompt": _FUNC_COMPLETION_PROMPT,
        "temperature": 0, "seed": 0},
    {"model": FAST_MODEL, "prompt": _FUNC_COMPLETION_PROMPT,
        "stop": _FUNC_COMPLETION_STOP, "n": 7, "seed": 0},
    {"model": DEFAULT_MODEL, "prompt": _FUNC_COMPLETION_PROMPT,
        "temperature": 0, "seed": 1},
    {"model": DEFAULT_MODEL, "prompt": _FUNC_COMPLETION_PROMPT,
        "stop": _FUNC_COMPLETION_STOP, "n": 2, "seed": 2},
    {"model": DEFAULT_MODEL, "prompt": _FUNC_COMPLETION_PROMPT,
        "stop": _FUNC_COMPLETION_STOP, "n": 1, "seed": 2},
]
...
def implement(
    ...
    assertions: Optional[Union[str, Callable[[str],
                                             Tuple[str, float]]]] = generate_assertions,
) -> Tuple[str, float]:
